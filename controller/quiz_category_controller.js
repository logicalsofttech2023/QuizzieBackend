  ...
  try {
    ...
    if (existingQuizCategory) {
      return res.json({
        code: 401,
        status: false,
        message: "Quiz Category with this title already exists",
      });
    }
    ...
    res.json({
      code: 201,
      status: true,
      message: "New Quiz category has been created",
      newQuizCategory: newQuizCategory,
    }); // Return the created quiz category as the response.
  } catch (err) {
    throw new Error(err);
  }
});
  ...
  try {
    ...
    if (!validateMongoDbId(quiz_category_id)) {
      return res.json({
        code: 400,
        status: false,
        message: "Invalid quiz_category_id format",
      });
    }
...
const deleteSpecificQuizCategory = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const deleteQuizCategory = await QuizCategory.findByIdAndDelete(
      quiz_category_id
    );
...
const updateQuizCategory = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.json({
      code: 200,
      status: true,
      message: "Quiz category details has been updated succefully",
      updatedQuizCategory: updatedQuizCategory,
    });
  } catch (err) {
    throw new Error(err);
  }
...
const getAllQuizzesFromQuizCategoryId = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!validateMongoDbId(quiz_category_id)) {
      return res
        .status(400)
        .json({ message: "Invalid quiz category id format" });
    }
    ...
    const quizzes = await Quiz.find({ category: quiz_category_id }).select(
      "-category"
    );
...
module.exports = {
  ...
  getSpecificQuizCategory,
  deleteSpecificQuizCategory,
  updateQuizCategory,
  getAllQuizzesFromQuizCategoryId,
};
